...
def Measures_all():
    ...
    print('NEW')
    l=6*fs #Segment length, all data at fs Hz
    ...
    assert all([co[i] == co[subjects[0]] for i in co]), 'data integrity?'
    co2 = Counter([x.split('_')[1].split('.')[0] for x in s])
    conditions = list(co2.keys())
    assert all([co2[i] == co2[conditions[0]] for i in co2]), 'data integrity?'

    path_res='/mnt/8cfe1683-d974-40f3-a20b-b217cad4722a/lucia/results'
    ...
    for condition in conditions:
        for subject in subjects:
            ...
            d = mat73.loadmat(path_dat+'/data/%s_%s.set' 
                              % (subject,condition))['data']
            
            shapes['%s_%s' % (subject,condition)] = d.shape                  
            chs,obs = d.shape
...
## don't save as txt, save as np.save, array

                if not os.path.exists(path_res+'/%s' %(subject)):
                    os.makedirs(path_res+'/%s' %(subject))
                if measure=='PSpec':
                    for jj in range(4):
                        np.save(path_res+'/%s/PSpec%s_%s.npy'
                                %(subject,jj,condition),S[:,:,jj])
                else:
                    np.save(path_res+'/%s/%s_%s.npy' %(subject,measure,condition),S)
            print(condition, subject, d.shape)
...
def dim_reduction():
    '''
    concatenate all data, run PCA, plot 2d figure colored by state
    (gradient in time) and marker by subject 
    (latex subject number $$3$$)
    '''
    ...
    #wsc = np.zeros((64,76800 * len(conditions) * len(subjects)))
    
    for subject in subjects:
        wsc = []
        for condition in conditions:
            d = mat73.loadmat(path_dat+'/data/%s_%s.set' 
                % (subject,condition))['data'][:-2,:76800]
            wsc.append(d)
                
        dat = np.concatenate(wsc,axis=1).T   
        pca = PCA(n_components = 3)
        wsc = pca.fit_transform(dat).T  # input: n_samples x n_features
        np.save('/mnt/8cfe1683-d974-40f3-a20b-b217cad4722a/'
                f'lucia/results/pca/{subject}.npy', wsc)
        print(subject, 'done')
  ...
  for condition in conditions:
   for subject in subjects:
    ...
    for t in range(int(obs/l)):
     s0=[]
     for jj in range(chs):
      s0.append(signal.welch(d[jj,l*t:l*(t+1)],fs))
     #print(condition, subject, measure, t,'of',trs)
